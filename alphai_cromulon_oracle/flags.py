...
def build_tensorflow_flags(model_config):
    ...
    tf.app.flags.DEFINE_boolean('use_gpu', model_config.get('use_gpu', False),
                                """Whether to use GPU formatting of convolutional layers. """)

    tf.app.flags.DEFINE_integer('n_res_blocks', model_config['n_res_blocks'], """Number of residual blocks.""")

    tf.app.flags.DEFINE_boolean('do_kernel_regularisation', model_config['do_kernel_regularisation'],
    ...
    tf.app.flags.DEFINE_integer('n_retrain_epochs', model_config['n_retrain_epochs'], """How many epochs to be used for re-training a previously stored cromulon.""")
